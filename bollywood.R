...
#	Import the Bollywood data set in Rstudio in a variable named bollywood
  ...
  bollywood<-read.csv("bollywood.csv",stringsAsFactors = F)
  View(bollywood)
  #reading csv file woth strings as factors set to false by default
...
# Store the names of those movies in last_10 vector (in the same order)
	...
	#using tail to get last 10 rows of the data frame
#Q2.
...
# Store the result in na_bollywood vector
	...
	#finding out the total number of NA's in bollywood data frame
...
# Store the movie name in variable named top_movie
	...
	top_movie<-bollywood$Movie[bollywood$Tcollection==max(bollywood$Tcollection,na.rm=TRUE)]
  #using max function to find max Tcollection
...
# Store the movie name in variable named top_2_movie
	...
	top_2_movie<-bollywood$Movie[bollywood$Tcollection==(sort(bollywood$Tcollection,decreasing = T)[2])]
	#sorting the data frame in descending order and selecting the second element
...
# You can use	a column named 'Tcollection' for this 
  ...
  shahrukh_Tcollection <- sum(shahrukh$Tcollection)
    
	akshay_Tcollection <- sum(akshay$Tcollection)
    
	amitabh_Tcollection <- sum(amitabh$Tcollection)

  #sum of all the TCollection in the shahrukh, akshay and amitabh subsets respectively
...
# Write command/s to find out how many movies are in Flop, Average, Hit and Superhit categories in the entire Bollywood data set.
  ...
  #converting the Verdict column to factor and using summary function to see the number of occurences of each Verdict factor in bollywood data frame
...
# Store the names of 4 movies in same sequence in movie_result vector
 ...
 #get the index of the movie record with the highest value in collection category and using sapply to apply it to all collection category columns
